...
**上游仓库没有 LICENSE 文件。** 按默认版权规则，这意味着上游作者保留全部权利，
公开分发衍生作品需要其授权。可行的做法：

1. 联系 xiaoxiao，请其为上游仓库补一个开源许可（MIT / Apache-2.0 之类），或书面同意本 fork 公开
2. 拿到之后再给本仓库加 LICENSE，并在其中如实写明代码含上游作者的著作权

另外两点与开源相关的事实，也需要在公开前处理：

- `manual/wiki/` 是维基百科（CC BY-SA）与百度百科（无授权）正文首段的缓存。
  产物页面里已经逐条标注来源并给出链接，但**把缓存本身放进公开仓库是另一回事**
