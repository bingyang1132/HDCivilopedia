...
> **状态：已冻结（2026-08-08）。** 逆向卡在块描述表（见下），而向作者要源图这条路也走不通——
> 有 mod 作者反馈美术资源一般不会外流。唯一还成立的方案是社区提的**程序化游戏内截图**：
> 在游戏里打开百科逐条截图，理想情况用 Lua 控制翻页。工作量大，暂不启动。
> 本文保留，是为了下次真要做时不用从零开始，尤其是「已排除的假设」那一节。
